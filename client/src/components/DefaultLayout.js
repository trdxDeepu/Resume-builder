...
function DefaultLayout(props) {
  ...
  const menu = (
    <Menu>
      ...
      <Menu.Item
        onClick={() => {
          localStorage.removeItem("Resume_build-user");
          navigate("/login");
        }}
      >
        <span>Logout</span>
  ...
  return (
    <div className="layout">
      <div className="header">
        ...
        <Dropdown overlay={menu} placement="bottomLeft" className="dropd">
          <Button className="btn" icon={<UserOutlined />}>{user.username}</Button>
        </Dropdown>
      </div>
